...
public final class SqlSessionUtils {
	...
	public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, ExecutorType executorType, PersistenceExceptionTranslator exceptionTranslator) {
		...
		// TODO XIEQJ
		registerSessionHolder(holder, sessionFactory, executorType, exceptionTranslator, session);
	 ...
	 */
	private static void registerSessionHolder(SqlSessionHolder holder,SqlSessionFactory sessionFactory, ExecutorType executorType,
			...
			if (environment.getTransactionFactory() instanceof SpringManagedTransactionFactory) {
				if (LOGGER.isDebugEnabled()) {
					...
					if(executorType==ExecutorType.REUSE){
						typeName="REUSE:";
					}else if(executorType==ExecutorType.BATCH){
						typeName="BATCH:";
					}else if(executorType==ExecutorType.SIMPLE){
						typeName="SIMPLE:";
					}
					LOGGER.debug("Registering transaction synchronization for SqlSession [" + typeName + session + "]");
				}
				if(holder==null){
					...
					holder.addSqlSession(executorType, session);
				}
				holder.requested();
			} else {
	...
	private static SqlSession sessionHolder(ExecutorType executorType, SqlSessionHolder holder) {
		...
		if (holder != null && holder.isSynchronizedWithTransaction()) {
			...
			session=holder.getSqlSession(executorType);
			if(session==null){
				return null;
			}
			holder.requested();
			...
			if (LOGGER.isDebugEnabled()) {
				...
				LOGGER.debug("Fetched SqlSession [" + typeName + session + "] from current transaction");
			}
	...
	public static void closeSqlSession(SqlSession session, SqlSessionFactory sessionFactory) {
		...
		SqlSessionHolder holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);
		if ((holder != null) && (holder.isHolded(session))) {
			if (LOGGER.isDebugEnabled()) {
	...
	public static boolean isSqlSessionTransactional(SqlSession session, SqlSessionFactory sessionFactory) {
		...
		return (holder != null) && (holder.isHolded(session));
	}
	...
	private static final class SqlSessionSynchronization extends TransactionSynchronizationAdapter {
		...
		public void suspend() {
			if (this.holderActive) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Transaction synchronization suspending SqlSession [" + this.holder.toString() + "]");
				}
		...
		public void resume() {
			if (this.holderActive) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Transaction synchronization resuming SqlSession [" + this.holder.toString() + "]");
				}
		...
		public void beforeCommit(boolean readOnly) {
			...
			if (TransactionSynchronizationManager.isActualTransactionActive()) {
				try {
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Transaction synchronization committing SqlSession [" + this.holder.toString() + "]");
					}
		...
		public void beforeCompletion() {
			...
			if (!this.holder.isOpen()) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Transaction synchronization deregistering SqlSession [" + this.holder.toString() + "]");
				}
				TransactionSynchronizationManager.unbindResource(sessionFactory);
				this.holderActive = false;
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Transaction synchronization closing SqlSession [" + this.holder.toString() + "]");
				}
				this.holder.close();
			}
		}
